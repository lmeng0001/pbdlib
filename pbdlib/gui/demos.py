...
class Robot(object):
    def __init__(self, T):
        self.x, self.dx, self.dt = np.array(
            [0., 0.]), np.array([0., 0.]), 1. / T
        self.ddx, self.fsx, self.fx = np.array(
...
class InteractiveDemos(Interactive, Robot):
    ...
    def __init__(self, filename='test', path='', **kwargs):
        ...
        self.path = os.path.dirname(
            pbd.__file__) + '/data/gui/' if path == '' else path
    ...
    def timer_event(self, event):
        if self.is_demonstrating:
            if self.curr_mouse_pos is None:
                self.pretty_print('Outside')
                return
    ...
    def move_event(self, event):
        self.curr_mouse_pos = None if None in [
            event.xdata, event.ydata] else np.array([event.xdata, event.ydata])

        if event.key == 'shift' or self.is_demonstrating:
            self.robot_pos = np.copy(self.curr_mouse_pos)

            if not self.is_demonstrating:
                self.plots['robot_plot'].set_data(
    ...
    def plot_timer_event(self, event):
        ...
        self.plots['attractor_plot'].set_data(
            self.robot_pos[0], self.robot_pos[1])
    ...
    def click_event(self, event):
        if event.key is None:
            ...
            if not self.velocity_mode:
                ...
                self.x = self.demos['x'][-1][0] if self.nb_demos > 0 else np.array([
                                                                                   0., 0.])
    ...
    def finish_demo(self):
        ...
        self.plots['demo_%d' % self.nb_demos] = self.ax_x.plot(
            curr_demo_arr[:, 0], curr_demo_arr[:, 1], lw=2, ls='--')[0]
        self.plots['demo_dx_%d' % self.nb_demos] = self.ax_dx.plot(
            curr_demo_dx_arr[:, 0], curr_demo_dx_arr[:, 1], lw=2, ls='--')[0]
    ...
    def save_demos(self):
        ...
        file_path = asksaveasfilename(
            initialdir=self.path, initialfile=self.filename + '.npy')
