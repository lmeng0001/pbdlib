...
def read_frames(bag, start_time=400, end_time=410):
    ...
    ts = []
    frames = ['vicon', 'vicon_wrist1', 'vicon_chest1',
              'vicon_wrist2', 'vicon_chest2'] + ['%d' % i for i in range(24)]
    # q = {s: [] for s in frames}
    # x = {s: [] for s in frames}
        ...
        if topic == '/tf':
            ...
            if frame_id in frames:
                ...
                if transf.header.frame_id != 'world':
                    t_mat_ = tf.transformations.translation_matrix(
                        [x_.x, x_.y, x_.z])
                    r_mat_ = tf.transformations.quaternion_matrix(
                        [q_.x, q_.y, q_.z, q_.w])
                    ...
                    x_.x, x_.y, x_.z = tf.transformations.translation_from_matrix(
                        m_t_mat)
                    q_.x, q_.y, q_.z, q_.w = tf.transformations.quaternion_from_matrix(
                        m_t_mat)
    ...
    for topic in list(demo_buffer.keys()):
        for feature in list(demo_buffer[topic].keys()):
            demo_buffer[topic][feature] = np.asarray(
                demo_buffer[topic][feature])
