    ...
    def get_seq(self, xi0, return_target=False):
        ...
        for t in range(1, self.horizon-1):
            ...
            if return_target:
                ...
                us += [-self._K[t].dot(xis[-1]) +
                       self._Kv[t].dot(self._v[t+1])]
    ...
    def ricatti(self, x0, n_best=None):
        ...
        for i in range(self.full_gmm_xi.nb_states):
            if n_best is not None and log_prob_components[i] < a:
                ...
                costs += [np.sum(self.gmm_u.log_prob(us) +
                                 self.full_gmm_xi.log_prob(xis))]
...
class PoGLQR(LQR):
    ...
    def seq_xi(self):
        if self._seq_xi is None:
            self._seq_xi = self.mvn_sol_xi.mu.reshape(
                self.horizon, self.xi_dim)
