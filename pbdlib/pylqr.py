...
class PyLQR(object):
    def __init__(self, A=None, B=None, dt=0.01, canonical=False, nb_dim=2, rFactor=-6,
        ...
        if canonical:
            if not discrete:
                ...
                for i in range(nb_deriv):
                    A1d += np.diag(np.ones(nb_deriv-i), i) * \
                        np.power(self.dt, i) / factorial(i)
            ...
            # only used if you want to track a force, by default a zero force is tracked
            self.force_target = np.zeros((nb_dim, horizon))
    ...
    def set_hmm_problem(self, model, q, horizon=None, force_model=None, use_force=True):
        ...
        if self.type in [LQR_type.canonical, LQR_type.standard]:
            self.Qp = np.zeros(
                (self.horizon, self.nb_dim * 2, self.nb_dim * 2))

            for i in range(horizon):
                self.target[0:self.nb_dim, i] = model.Mu[0:self.nb_dim, q[i]]
                Qtmp[0:self.nb_dim, 0:self.nb_dim] = model.Lambda[0:self.nb_dim,
                                                                  0:self.nb_dim, q[i]]
